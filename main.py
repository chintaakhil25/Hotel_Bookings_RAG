...
from rag_system import (load_data_and_create_documents, create_vector_store,
                         ...
                         generate_answer, infer_filter_type)
...
def ask_question(query: Query):
    ...
    try:
        # Infer filter_type (if any)
        filter_type = infer_filter_type(query.text)

        # Retrieve insights, using the inferred filter_type
        results = retrieve_insights(vector_store,query.text, k=3, filter_type=filter_type) #Defaults to k=3
        context = ""
